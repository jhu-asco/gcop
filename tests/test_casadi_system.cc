...
TEST(EigenConversionToDM, Vector3d) {
  ...
  input(1) = 1.0;
  casadi::DM output = gcop::CasadiSystem<>::convertEigenToDM(input);
...
TEST(DMConversionToEigen, Vector3d) {
  ...
  Eigen::Vector3d output = gcop::CasadiSystem<>::convertDMToEigen(input);
...
TEST(DMConversionToEigen, MatrixXd) {
  ...
  Eigen::MatrixXd output = gcop::CasadiSystem<>::convertDMToEigen(input);
  std::vector<double> elems = input.get_elements();
