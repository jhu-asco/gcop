...
void MbsTranslate::q2transform(geometry_msgs::Transform &transformmsg, Vector6d &bpose)
{
	tf::Quaternion q;
...
void MbsTranslate::xml2vec(VectorXd &vec, XmlRpc::XmlRpcValue &my_list)
{
...
*/
void MbsTranslate::paramreqcallback(gcop_ctrl::MbsDMocInterfaceConfig &config, uint32_t level) 
{
	int nb = mbsmodel->nb;
	Nit = config.Nit; 
	//int N = config.N;
