...
const float TRIANGLE_SIZE=20.f;
  ...
  */
  ...
  cout<<"Recording Sensor Data"<<endl;
  {
    ...
    { 
      //Set zsi:
      zs[i]<<brccar->x[0],0,brccar->x[1];
      if (brccar)
      {
        cout<<"u: "<<us[i].transpose()<<endl;
        brccar->Step2(us[i], h);//Move
      }
      printf("Zs:[%d]: %f\t%f\n",i, zs[i](0), zs[i](2));
    }
    ...
    case 'i' :
      {
        ...
        if(gn)
        {
          ...
          /*for(int i =0; i < N;++i)
          {
            cout<<"Zsdiff["<<i<<"]: "<<(zs[i] - (gn->zs)[i]).transpose()<<endl;
            cout<<"Zstrue["<<i<<"]: "<<zs[i].transpose()<<endl;
            cout<<"gn->Zs["<<i<<"]: "<<(gn->zs)[i].transpose()<<endl;
          }
          getchar();
          */
