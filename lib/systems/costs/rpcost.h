#ifndef GCOP_RPCOST_H
#define GCOP_RPCOST_H
...
namespace gcop {
  ...
  template <typename T, 
    int _nx = Dynamic, 
    int _nu = Dynamic,
    int _np = Dynamic> class RPCost : public Cost<T, _nx, _nu, _np> {
  public:
   ...
   */
  RPCost(System<T, _nx, _nu, _np> &sys, double tf);
  ...
  const Vector3d e = Vector3d::UnitZ();
  double gain;
  //bool useFinite;
  ...
  template <typename T, int _nx, int _nu, int _np> 
    double RPCost<T, _nx, _nu, _np>::simple_c(const T& x) {

    return (gain*(1 - (x.R*e).dot(e)));
  }
  template <typename T, int _nx, int _nu, int _np>
    void RPCost<T, _nx, _nu, _np>::pop_Lx(const T& x, Matrix<double, _nx, 1> *Lx){
      Matrix3d ehat;
      SO3::Instance().hat(ehat, e);
      Vector3d dLdR = gain*(ehat.transpose()*x.R.transpose()*e);
      Lx->setZero();
      Lx->head(3) = dLdR;
      /*double c = simple_c(x);
      double eps = 1e-10;
  ...
  template <typename T, int _nx, int _nu, int _np> 
    RPCost<T, _nx, _nu, _np>::RPCost(System<T, _nx, _nu, _np> &sys, 
                                       double tf) : 
       Cost<T, _nx, _nu, _np>(sys, tf), gain(1) {
  }  
  
  template <typename T, int _nx, int _nu, int _np> 
    double RPCost<T, _nx, _nu, _np>::L(double t, const T& x, const Vectorcd& u, double h,
                                          const Vectormd *p,
