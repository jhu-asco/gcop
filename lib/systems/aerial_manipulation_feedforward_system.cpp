...
MX AerialManipulationFeedforwardSystem::casadi_step(MX t, MX h, MX xa, MX u,
  ...
  // State
  std::vector<MX> x_splits = MX::vertsplit(xa, {0, 15, 17, 19, 21});
  MX quad_states = x_splits.at(0);
  MX joint_angles = x_splits.at(1);
  MX joint_velocities = x_splits.at(2);
  MX joint_angles_desired = x_splits.at(3);
  // Quad dynamics
  ...
  // Resulting state
  MX xb =
      vertcat(std::vector<MX>{quad_xb, joint_angles_next, joint_velocities_next,
