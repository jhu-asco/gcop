...
namespace gcop {
  ...
  template <typename T, int nx, int nu, int np, int _ntp> 
    FlatOutputTparam<T, nx, nu, np, _ntp>::FlatOutputTparam(System<T, nx, nu, np> &sys, int ny_, int numberofknots_, int numberofderivatives_) :  Tparam<T, nx, nu, np, _ntp>(sys, numberofknots_*ny),ny(ny_), numberofderivatives(numberofderivatives_), numberofknots(numberofknots_)  {
  ...
  template <typename T, int nx, int nu, int np, int _ntp> 
    void FlatOutputTparam<T, nx, nu, np, _ntp>::To(Vectorntpd &s, 
			...
			{
				VectorXd flatoutput = flatoutputs.segment(count_samples*ny,ny);
				(this->sys).StateAndControlsToFlat(flatoutput, xs[count_samples], us[count_samples]);
			}
			//Tail or final state:
			{
				VectorXd flatoutput = flatoutputs.tail(ny);
			...
			for(int count_knots =0; count_knots < numberofknots; count_knots++)
			{
				...
				for(int count_samples =0; count_samples <= N; count_samples++)
				{
					...
					basis.block(count_samples*ny, count_knots,ny,1) = (DeCasteljau(knotvector, (ts[count_samples]/tf), 0,numberofknots-1));//The evaluation using P = e_i will be B_n,i(u_i)
				}
			}
